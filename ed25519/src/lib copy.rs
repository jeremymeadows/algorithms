...
static G: SyncLazy<Point> = SyncLazy::new(|| {
    let y = (BigInt::from(4) * modp_inv(&BigInt::from(5))).modulo(&P);
    let x = recover_x(&y, false).unwrap();
...
fn modp_inv(x: &BigInt) -> BigInt {
    x.modpow(&(P.clone() - 2), &P)
}

#[inline]
fn sha512_modq(bytes: &[u8]) -> BigInt {
    BigInt::from_le_bytes(&Sha512::hash(bytes).as_bytes()).modulo(&Q)
}
    ...
    ($t:ty) => {
        impl Add<$t> for Point {
            ...
            fn add(self, other: $t) -> Self::Output {
                let a = ((&self.y - &self.x) * (&other.y - &other.x)).modulo(&P);
                let b = ((self.y + self.x) * (&other.y + &other.x)).modulo(&P);
                let c = (BigInt::from(2) * self.t * &other.t * &*D).modulo(&P);
                let d = (BigInt::from(2) * self.z * &other.z).modulo(&P);
    ...
    ($t:ty) => {
        impl Mul<$t> for Point {
            ...
            fn mul(mut self, s: $t) -> Self::Output {
                ...
                while s > BigInt::zero() {
                    if s.clone() & BigInt::one() != BigInt::zero() {
                        other += self.clone();
                    }
                    self += self.clone();
                    s = s >> 1;
...
fn recover_x(y: &BigInt, signed: bool) -> Option<BigInt> {
    if y >= &*P {
        return None;
    }

    let y2 = y * y;
    let x2: BigInt = (y2.clone() - 1) * modp_inv(&(&*D * y2 + 1));
    ...
    if ((&x * &x) - &x2).modulo(&*P) != BigInt::zero() {
        x = (&x * BigInt::from(2).modpow(&((P.clone() - 1) / 4), &P)).modulo(&P);
    }
    if (&x * &x - &x2).modulo(&P) != BigInt::zero() {
        return None;
    }

    if (x.to_le_bytes().iter().next().unwrap() & 1) as u8 != if signed { 1 } else { 0 }
    {
        x = P.clone() - x;
    }
    Some(x)
...
fn point_compress(p: &Point) -> Vec<u8> {
    ...
    let x = (&p.x * &z_inv).modulo(&P);
    let y = (&p.y * &z_inv).modulo(&P);
...
fn point_decompress(bytes: &[u8]) -> Result<Point, &str> {
    ...
    let mut y = BigInt::from_le_bytes(bytes);
...
fn secret_expand(secret: &[u8; 32]) -> (BigInt, Vec<u8>) {
    ...
    let mut a = BigInt::from_le_bytes(&h[0..32]);
...
pub fn verify(public: &[u8], msg: &[u8], signature: &[u8]) -> bool {
    ...
    let s = BigInt::from_le_bytes(&signature[32..]);
    if s >= *Q {
        return false;
    }
    ...
    ((&p.x * &q.z) - (&q.x * &p.z).modulo(&P)) | ((&p.y * &q.z) - (&q.y * &p.z).modulo(&P))
        != BigInt::zero()
}
...
pub fn ecdh(a: &[u8], b: &[u8]) -> Vec<u8> {
    point_compress(
        &(point_decompress(&b).unwrap() * BigInt::from_le_bytes(&a)),
    )
}
...
mod tests {
    ...
    fn test_ecdh() {
        ...
        let int_a = BigInt::from_le_bytes(&secret_a);
        let int_b = BigInt::from_le_bytes(&secret_b);
        ...
        assert_eq!(
            ...
            ((G.clone() * &int_b) * &int_a).x.modulo(&D),
            ((G.clone() * &int_a) * &int_b).x.modulo(&D),
        );
