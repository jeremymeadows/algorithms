...
/// A SHA-1 hasher which can digest data and return an 80-bit value.
///
/// # Examples
/// ```
/// use sha::{Digest, Sha, sha1::Sha1};
///
...
/// hasher.add(b" world");
///
/// let digest = hasher.digest(); // 2aae6c35c94fcfb415dbe95f408b9ce91ee846ed
/// assert_eq!(
///     digest.as_bytes(),
///     [
///         42, 174, 108, 53, 201, 79, 207, 180, 21, 219,
///         233, 95, 64, 139, 156, 233, 30, 232, 70, 237,
///     ]
/// );
///
/// // or use the `hash` function to get the digest of a byte slice
/// assert_eq!(digest, Sha1::hash(b"hello world"));
/// ```
pub struct Sha1 {
    data: Vec<u8>,
    h: [u32; 5],
...
impl_digest!(Sha1Digest for Sha1);

impl Sha<Sha1Digest> for Sha1 {
    ...
    fn hmac(key: &[u8], data: &[u8]) -> Sha1Digest {
        let mut hasher = Sha1::new();
        hasher.add(data);
    ...
    fn digest_hmac(&mut self, key: &[u8]) -> Sha1Digest {
...
impl Sha1 {
    ...
    fn pad(&self) -> Vec<u8> {
        ...
        padding.append(
            &mut ([0x00_u8].repeat(
                Self::BLOCK_SIZE
                    - 1
                    - (self.data.len() + u64::BITS as usize / 8) % Self::BLOCK_SIZE,
            )),
        );
    ...
    fn expand_block(block: &[u8; Self::BLOCK_SIZE]) -> [u32; 80] {
        let mut w = [0; 80];
