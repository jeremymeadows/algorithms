...
use crate::{impl_digest, Digest, Sha};

/// A SHA-2 hasher which can digest data and return an 512-bit value.
///
/// # Examples
/// ```
/// use sha::{Digest, Sha, sha512::Sha512};
///
/// let mut hasher = Sha512::new();
///
/// // incrementally add data to the hasher
/// hasher.add(b"hello");
/// hasher.add(b" world");
///
/// let digest = hasher.digest(); // 309ecc489c12d6eb4cc40f50c902f2b4d0ed77ee511a7c7a9bcd3ca86d4cd86f989dd35bc5ff499670da34255b45b0cfd830e81f605dcf7dc5542e93ae9cd76f
/// assert_eq!(
///     digest.as_bytes(),
///     [
///         48, 158, 204, 72, 156, 18, 214, 235, 76, 196, 15, 80, 201, 2, 242, 180, 208, 237, 119,
///         238, 81, 26, 124, 122, 155, 205, 60, 168, 109, 76, 216, 111, 152, 157, 211, 91, 197,
///         255, 73, 150, 112, 218, 52, 37, 91, 69, 176, 207, 216, 48, 232, 31, 96, 93, 207, 125,
///         197, 84, 46, 147, 174, 156, 215, 111,
///     ]
/// );
/// 
/// // or use the `hash` function to get the digest of a byte slice
/// assert_eq!(digest, Sha512::hash(b"hello world"));
/// ```
pub struct Sha512 {
    data: Vec<u8>,
    h: [u64; 8],
...
impl_digest!(Sha512Digest for Sha512);

impl Sha<Sha512Digest> for Sha512 {
    ...
    fn hmac(key: &[u8], data: &[u8]) -> Sha512Digest {
        let mut hasher = Sha512::new();
        hasher.add(data);
    ...
    fn digest(&mut self) -> Sha512Digest {
    ...
    fn digest_hmac(&mut self, key: &[u8]) -> Sha512Digest {
        let mut key = key.to_vec();

        if key.len() > Self::BLOCK_SIZE {
            key = Self::hash(&key).to_vec();
        }

        while key.len() < Self::BLOCK_SIZE {
            key.push(0);
        }

        let mut i_pad = [0x36_u8; Self::BLOCK_SIZE];
        for i in 0..Self::BLOCK_SIZE {
            i_pad[i] ^= key[i];
        }

        let mut o_pad = [0x5c_u8; Self::BLOCK_SIZE];
        for i in 0..Self::BLOCK_SIZE {
            o_pad[i] ^= key[i];
        }
        ...
        let h: [u64; 8] = Self::hash(
            &[
                &o_pad,
                Self::hash(&[&i_pad, self.data.as_slice()].concat()).as_slice(),
            ]
            .concat(),
        )
        .chunks(8)
        ...
        [h[0], h[1], h[2], h[3], h[4], h[5], h[6], h[7]].into()
    }
}
...
impl Sha512 {
    ...
    fn pad(&self) -> Vec<u8> {
        ...
        padding.append(
            &mut ([0x00_u8].repeat(
                Self::BLOCK_SIZE
                    - 1
                    - (self.data.len() + u128::BITS as usize / 8) % Self::BLOCK_SIZE,
            )),
        );
    ...
    fn blocks(&self) -> Vec<[u8; Self::BLOCK_SIZE]> {
        let mut a = [0; Self::BLOCK_SIZE];
        let mut v = Vec::new();

        for i in self.pad().chunks(Self::BLOCK_SIZE).collect::<Vec<_>>() {
            for j in 0..a.len() {
                a[j] = i[j];
            }
            v.push(a.clone());
    ...
    fn expand_block(block: &[u8; Self::BLOCK_SIZE]) -> [u64; 80] {
        let mut w = [0; 80];
